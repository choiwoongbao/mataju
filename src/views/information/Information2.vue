...
const sizes = [
  { key:'mini', title:'미니', dim:'65×50×90',
  ...
  { key:'small', title:'스몰', dim:'75×60×90',
  ...
  { key:'medium', title:'미디움', dim:'95×70×90',
  ...
  { key:'large', title:'라지', dim:'100×90×240',
  ...
  { key:'max', title:'맥스라지', dim:'120×100×240',
...
/* @media (max-width:420px){
  .size-strip{ grid-template-columns:repeat(auto-fit, minmax(110px, 1fr)); }
...
.spec .name{ margin:2px 0 12px; font-size:22px; font-weight:900; letter-spacing:-.01em; }
.rows{ display:grid; gap:10px; }
.rows > div{
  display:grid; grid-template-columns:110px 1fr; gap:10px; align-items:center;
}
.rows dt{ color:#667085; font-size:13px; font-weight:700; }
.rows dd{ margin:0; font-size:14px; color:#0f172a; }

.chips{ list-style:none; padding:0; margin:0; display:flex; flex-wrap:wrap; gap:8px; }
.chips li{
  padding:6px 10px; border-radius:999px; background:#f3f4f6; font-size:12px;
  border:1px solid #e5e7eb;
}

/* CTA 버튼 — 브라우저 기본 모양 제거 + 필 버튼 */
.ship-cta{ margin-top:16px; display:flex; align-items:center; gap:10px; font-size:14px; }
.ship-cta .q{ color:#0f172a; font-weight:800; }
.link-cta{
...
@media (max-width:600px){
  ...
  .limit-inline .badge{
    ...
    font-size:11px; font-weight:800; color:#fff; background:#0ea5b6; border-radius:999px;
  }
...
/* ====== 참고 박스 ====== */
.notes{
  position:relative; margin:12px 0 0; padding:12px 14px;
  background:#f8fafc; border:1px dashed #cbd5e1; border-radius:12px;
  color:#475569; font-size:12.5px; line-height:1.6; list-style:none;
}
.notes::before{
...
.table-wrap{
  ...
  box-shadow: 0 1px 0 rgba(15,23,42,.03);
  margin-top:20px;
  margin-bottom:20px; /* ✅ 요금표와 참고박스 사이 여백 */
}

/* ====== 참고 박스 ====== */
.notes{
  position:relative;
  margin-top:30px; /* ✅ 요금표와의 간격 */
  margin-bottom: 30px;
  padding:14px 16px;
  background:#f8fafc;
